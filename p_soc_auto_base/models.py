...
class MuteableModel(BaseModel, models.Model):
    """
    Abstract model class that extends :class:`BaseModel` with support for
    muting alerts from this model.

    Mutes are used to stop alerts being generated about a given object. Alerts
    are generated by the `SOC Automation Server` based on the results of
    various tests that are performed on various objects we are interested in.
    If it is known that a specific object will be operating at a substandard
    level, it can be useful to block notifications about that object for a
    short period.
    """
    mute_until = models.DateTimeField(
        _('Do not send alerts until'), null=True, blank=True)
    """
    Before raising alerts we check if the current time is before this time,
    and if it is the alert is not raised.
    """
